...
void test(std::string name)
{
  ...
  flo::device::iterative::similarity_xform(d_D, d_xform, 1e-7);
...
#define FLO_SIMILARITY_XFORM_ITERATIVE_TEST(NAME) \
  TEST(SimilarityXformIterative, NAME)            \
  {                                               \
    test(#NAME);                                  \
  }

FLO_SIMILARITY_XFORM_ITERATIVE_TEST(cube)
FLO_SIMILARITY_XFORM_ITERATIVE_TEST(spot)
FLO_SIMILARITY_XFORM_ITERATIVE_TEST(bunny)

#undef FLO_SIMILARITY_XFORM_ITERATIVE_TEST
