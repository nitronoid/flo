...
#define FLO_SPIN_POSITIONS_ITERATIVE_DEVICE_BENCHMARK(NAME)                      \
  static void DEVICE_spin_positions_iterative_##NAME(benchmark::State& state) \
  {                                                                           \
    bench_impl(#NAME, state);                                                 \
  }                                                                           \
  BENCHMARK(DEVICE_spin_positions_iterative_##NAME);

FLO_SPIN_POSITIONS_ITERATIVE_DEVICE_BENCHMARK(spot)
FLO_SPIN_POSITIONS_ITERATIVE_DEVICE_BENCHMARK(bunny)
FLO_SPIN_POSITIONS_ITERATIVE_DEVICE_BENCHMARK(tyra)

#undef FLO_SPIN_POSITIONS_ITERATIVE_DEVICE_BENCHMARK
