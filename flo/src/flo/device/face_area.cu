          ...
          cusp::array1d<real, cusp::device_memory>::view do_face_area)
{
  ...
  auto fit = thrust::make_zip_iterator(
    thrust::make_tuple(di_faces.values.begin() + nfaces * 0,
                       di_faces.values.begin() + nfaces * 1,
                       di_faces.values.begin() + nfaces * 2));

  const int nvertices = di_vertices.num_cols;
  auto vit = thrust::make_zip_iterator(
    thrust::make_tuple(di_vertices.values.begin() + nvertices * 0,
                       di_vertices.values.begin() + nvertices * 1,
                       di_vertices.values.begin() + nvertices * 2));

  thrust::transform(
    fit,
    fit + nfaces,
    do_face_area.begin(),
    [vit = vit] __device__(const thrust::tuple<int, int, int>& face) {
      real3 edge0, edge1;
      {
